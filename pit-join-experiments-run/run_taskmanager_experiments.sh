...
# ../pit-joins-flink-thesis/datasets/partitioned/sorted/rows_per_cid_1/100000_80_8
JOIN_TYPE=$1
DATASET_PATH="../pit-joins-flink-thesis/datasets/partitioned/sorted/rows_per_cid_1/100000_80_8"
TASKMANAGER_RESULT_PATH="./results_taskmanager/tm4/${JOIN_TYPE}"
...
../flink-1.17.2/bin/start-cluster.sh
../flink-1.17.2/bin/start-cluster.sh
mkdir -p $TASKMANAGER_RESULT_PATH

../flink-1.17.2/bin/flink run ../pit-join-jar-builder/jars/partitioned/${JOIN_TYPE}_join.jar

curl localhost:8081/jobs/overview > "${TASKMANAGER_RESULT_PATH}/overview.json"
for i in $(jq '.jobs[] | .jid' "${TASKMANAGER_RESULT_PATH}/overview.json"); do
    temp="${i%\"}"
    temp="${temp#\"}"
    curl "localhost:8081/jobs/${temp}" > "${TASKMANAGER_RESULT_PATH}/${temp}.json"
done
